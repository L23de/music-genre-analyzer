...
treeModel <- C5.0(x = train[-c(2)], y = train$genre)
summary(treeModel)
plot(treeModel)
ruleModel <- C5.0(genre ~ ., data = train, rules = TRUE)
```
...
#training$p.2
train$genre <- NULL 
modelC5 <- C5.0( x = train[], as.factor(train$genre), method = "class") 
# x = all attributes, y = factor of what you want to predict
train$genre <- as.factor(train$genre)
#modelC5 <- C5.0( p~., data = training, rules = TRUE, method = "class")
summary(modelC5)
plot(modelC5)
#testing data with the trained data create
vpred <- predict(modelC5, test, type="class")
tabC5 <- table(data.frame(test$genre, pred = vpred))
tabC5
accuracyC5 <- sum(diag(tab))/sum(tab)
accuracyC5
```
